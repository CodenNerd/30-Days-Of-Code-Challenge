...
api.post('/createdata', (req, res)=>{
    ...
    if(!data.words[0]) return res.status(400).send({'error': 'words must be an array'}) // ensure words property array has atleast an element
    Helper.persistData('./db/day-14_db.txt', JSON.stringify(data), saved=>{       // I created a Helper object just to make things neater
...
api.get('/getdata', (req, res)=>{
   Helper.readData('./db/day-14_db.txt', (data)=>{
    if (!data) return res.status(404).send({'error':'Could not retrieve any content'});
        ...
        let {words} = data; // destructures words from data object

        words = words.filter(w=>Helper.isPalindrome(w));    
